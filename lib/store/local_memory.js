...
class LocalMemoryOffsetStore extends Base {
   ...
   * @return {Map<string, {messageQueue: MessageQueue, offset: number}>} MessageQueue -> offset
   */
  cloneOffsetTable(mqs) {
    const cloneOffsetTable = new Map();
    for (const [ key, offset ] of this.offsetTable) {
      const mq = mqs.find(q => q.key === key);
      if (!mq) {
        continue;
      }
      cloneOffsetTable.put(mq.key, { messageQueue: mq, offset });
    }
