...
class MQProducer extends ClientConfig {
  ...
  updateTopicPublishInfo(topic, info) {
    if (topic && info) {
      ...
      if (prev) {
        ...
        // modify by zhangbing
        this.logger.debug('[mq:producer] updateTopicPublishInfo() prev is not null, %j', prev);
        // end modify
      }
    }
  }
  ...
  async tryToFindTopicPublishInfo(topic) {
    ...
    if (topicPublishInfo && (topicPublishInfo.haveTopicRouterInfo || topicPublishInfo.ok)) {
