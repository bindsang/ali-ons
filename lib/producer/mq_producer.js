...
class MQProducer extends ClientConfig {
   ...
   * @param {boolean} autoInitial 控制是否自动初始化，producer
   * @constructor
   */
  constructor(options, autoInitial = true) {
    // ----- modify by zhangbing begin -------
    // 如果设置autoInitial = false，则不执行初始化
    options = Object.assign({}, defaultOptions, options);
    if (!autoInitial && options.initMethod) {
      delete options.initMethod;
    } else if (autoInitial && !options.initMethod) {
      options.initMethod = 'init';
    }
    super(options);
  ...
  /**
   * start the producer
   * @param {boolean} startFactory -
   * @return {void}
   */
  async init(startFactory = true) {
