...
class ConsumeMessageOrderlyService {
  ...
  async consumeMessageDirectly(msg, brokerName) {
    ...
    try {
      ...
      this._logger.warn(`consumeMessageDirectly exception: ${result.remark} Group: ${this.consumerGroup} Msgs: ${msg.msgId} MQ: ${messageQueue.key}`,
        err);
    }
  ...
  async submitConsumeRequest(msg, processQueue, messageQueue) {
    if (processQueue.dropped) {
      this._logger.warn(`run, the message queue not be able to consume, because it's dropped. ${messageQueue.key}`);
      return;
    }
    ...
    if (this._defaultMQPushConsumer.messageModel === MessageModel.BROADCASTING
      ...
      while (continueConsume) {
        if (processQueue.dropped) {
          this._logger.warn(`the message queue not be able to consume, because it's dropped. ${messageQueue.key}`);
          break;
        }

        if (this._defaultMQPushConsumer.messageModel === MessageModel.CLUSTERING
          && !processQueue.locked) {
          this._logger.warn(`the message queue not locked, so consume later, ${messageQueue.key}`);
        ...
        if (this._defaultMQPushConsumer.messageModel === MessageModel.CLUSTERING
          ...
          this._logger.warn(`the message queue lock expired, so consume later, ${messageQueue.key}`);
          await this._tryLockLaterAndReconsume(messageQueue, processQueue, 10);
          break;
        }
        ...
        if (msg) {
          ...
          try {
            ...
            if (processQueue.dropped) {
              this._logger.warn(`consumeMessage, the message queue not be able to consume, because it's dropped. ${messageQueue.key}`);
              break;
            }
            ...
            this._logger.warn(
              `orderly message exception: ${err.message}, Group: ${this.consumerGroup}, MQ: ${messageQueue.key}`,
              err);
          ...
          if (status !== ConsumeOrderlyStatus.SUCCESS) {
            ...
            this._logger.warn(`consumeMessage Orderly return not OK, status: ${status}, Group: ${this.consumerGroup} Msgs: ${msg.msgId} MQ: ${messageQueue.key}`);
          }
          ...
          continueConsume = await this._processConsumeResult(status, msg, processQueue, messageQueue, -1);
        } else {
  ...
  async _sendMessageBack(msg) {
    try {
      ...
      this._logger.error('sendMessageBack exception, group: ' + this.consumerGroup + ' msg: ' + msg.toString(), err);
    }
    return false;
  }
}
