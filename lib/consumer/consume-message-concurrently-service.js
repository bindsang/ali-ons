...
module.exports = class ConsumeMessageConcurrentlyService {
  ...
  async submitConsumeRequest(msg, processQueue, messageQueue) {
    if (this.processQueue.dropped) {
      this._logger.info(`the message queue not be able to consume, because it's dropped. group=${this._defaultMQPushConsumer.consumerGroup} ${this.messageQueue.key}`);
      return;
    }
    /**
     * @type {string}
    ...
    if (!processQueue.dropped) {
      await this._processConsumeResult(status, msg, processQueue, messageQueue, this);
    } else {
  ...
  async _sendMessageBack(msg, processQueue) {
    ...
    try {
      ...
      this.logger.error(`sendMessageBack exception, group: ${this._defaultMQPushConsumer.consumerGroup} msg: ${msg.msgId}`, err);
    }
