...
module.exports = class ConsumeMessageConcurrentlyService {
  ...
  async submitConsumeRequest(msg, processQueue, messageQueue) {
    if (processQueue.dropped) {
      this.logger.info(`the message queue not be able to consume, because it's dropped. group=${this.consumerGroup} ${this.messageQueue.key}`);
      return;
    }
    /**
     * @type {string}
    ...
    if (status !== ConsumeConcurrentlyStatus.CONSUME_SUCCESS) {
      ...
      this.logger.warn(`consumeMessage Concurrently return not OK, status: ${status}, Group: ${this.consumerGroup} Msgs: ${msg.msgId} MQ: ${messageQueue.key}`);
    }
