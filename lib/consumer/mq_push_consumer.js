...
class MQPushConsumer extends ClientConfig {
  ...
  async executePullRequestImmediately(messageQueue) {
    ...
    if (this.isPause()) {
      ...
      this.executePullRequestLater(pullRequest, this.options.pullTimeDelayMillsWhenSuspend);
      return;
    }
    ...
    if (!this.consumeOrderly) {
      ...
      if (processQueue.locked) {
        if (!pullRequest.lockedFirst) {
          const offset = this.computePullFromWhere(pullRequest.messageQueue);
          const brokerBusy = offset < pullRequest.nextOffset;
          this.logger.info('the first time to pull message, so fix offset from broker. pullRequest: {} NewOffset: {} brokerBusy: {}',
            pullRequest, offset, brokerBusy);
          if (brokerBusy) {
            this.logger.info('[NOTIFYME]the first time to pull message, but pull request offset larger than broker consume offset. pullRequest: {} NewOffset: {}',
              pullRequest, offset);
          }
        ...
        this.executePullRequestLater(pullRequest, this.options.pullTimeDelayMillsWhenException);
  ...
  consumerRunningInfo() {
    ...
    const prop = Object.assign({}, this.options);
    prop.PROP_CONSUME_ORDERLY = String(this._consumeOrderly);
    ...
    const subSet = [ ...this.subscriptions.values() ];
    ...
    for (const [ key, item ] of this._processQueueTable) {
      ...
      const pqinfo = { mqTable: { }, statusTable: {} };
      pqinfo.commitOffset(this._offsetStore.readOffset(mq, ReadOffsetType.MEMORY_FIRST_THEN_STORE));
