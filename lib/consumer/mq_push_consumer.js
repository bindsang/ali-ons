...
class MQPushConsumer extends ClientConfig {
  ...
  /**
   * subscribe
   * @param {String} [subscribeOptions] - options
   * @param {String} [subscribeOptions.topic] - topic
   * @param {String} [subscribeOptions.subExpression] - tag
   * @param {Function} [subscribeOptions.handler] - message handler
   * @param {boolean} [subscribeOptions.isOrder] - is order consumer
   * @return {void}
   */
  subscribe(subscribeOptions) {
    const { topic, subExpression, handler, isOrder = false } = subscribeOptions;
    ...
    if (this.messageModel === MessageModel.CLUSTERING) {
      ...
      this.consumeTopic(retrySubscriptionData, handler);
    }

    // ----------- modify by zhangbing begin -----------
    this.consumeTopic(subscriptionData, handler);
   ...
   */
  async consumeTopic(subscriptionData, handler) {
            ...
            if (item) {
              ...
              while (pq.msgCount) {
                ...
                if (!msg.tags || !needFilter || tagsSet.includes(msg.tags)) {
                  // ----------- modify by zhangbing begin -----------
                  await this._consumeMessageService.submitConsumeRequest(msg, pq, mq, handler);
                  // ----------- modify by zhangbing end -----------
                } else {
    ...
    if (findBrokerResult) {
      ...
      try {
        ...
        for (const mmqq of lockedMq) {
          const processQueue = this._processQueueTable.get(mmqq.key);
          if (processQueue) {
            processQueue.locked = true;
            processQueue.lastLockTimestamp = Date.now();
          }
