...
const defaultOptions = {
  ...
  postSubscriptionWhenPull: false, // 是否每次拉消息时，都上传订阅关系
...
class MQPushConsumer extends ClientConfig {
  ...
  get consumeConcurrentMin() {
    return this.options.consumeConcurrentMin;
  }
  ...
  async executePullRequestImmediately(messageQueue) {
    ...
    // flow control
    const cachedMessageCount = processQueue.msgCount;
    const cachedMessageSizeInMiB = Math.floor(processQueue.msgSize / (1024 * 1024));

    if (cachedMessageCount > this.options.pullThresholdForQueue) {
      if ((this._queueFlowControlTimes++ % 1000) === 0) {
        this.logger.warn(
          ...
          `maxOffset=${processQueue.msgList[processQueue.msgList.length - 1].queueOffset}, count=${cachedMessageCount}, size=${cachedMessageSizeInMiB} MiB, pullRequest=${pullRequest}, flowControlTimes=${this.queueFlowControlTimes}`);
      }
    ...
    if (cachedMessageSizeInMiB > this.options.pullThresholdSizeForQueue) {
      if ((this._queueFlowControlTimes++ % 1000) === 0) {
        this.logger.warn(
          ...
          `so do flow control, minOffset=${processQueue.msgList[0].queueOffset}, ` +
          `maxOffset=${processQueue.msgList[processQueue.msgList.length - 1].queueOffset}, count=${cachedMessageCount}, ` +
    ...
    if (!this.consumeOrderly) {
      if (processQueue.maxSpan > this.options.consumeConcurrentlyMaxSpan) {
        ...
        //   this.logger.warn(
        //     `the queue's messages, span too long, so do flow control, minOffset=${processQueue.msgList[0].queueOffset}, maxOffset=${processQueue.msgList[processQueue.msgList.length - 1].queueOffset}, maxSpan=${processQueue.maxSpan}, pullRequest=${pullRequest}, flowControlTimes=${queueMaxSpanFlowControlTimes}`);
        // }
        return;
      }
    } else {
    ...
    switch (pullResult.pullStatus) {
      case PullStatus.FOUND:
      {
        ...
        // submit to consumer
        const dispathToConsume = processQueue.putMessage(pullResult.msgFoundList);
        this._consumeMessageService.submitConsumeRequest(
  ...
  async pullKernelImpl(messageQueue, subExpression, subVersion, offset, commitOffset) {
    ...
    if (subscriptionData.tagsSet.length > 0 && !subscriptionData.classFilterMode) {
      pullResult.msgFoundList = pullResult.msgFoundList.filter(msg => !msg.tags || subscriptionData.tagsSet.includes(msg.tags));
    }
   ...
   * @return {void}
   */
  async removeUnnecessaryMessageQueue(mq, pq) {
    await this._offsetStore.persist(mq);
    ...
    if (this._consumeOrderly && this.messageModel === MessageModel.CLUSTERING) {
      try {
        if (await pq.lockConsume.tryLock(1000)) {
          try {
            const success = await this.unlockDelay(mq, pq);
            return success;
          } finally {
            pq.lockConsume.unlock();
          }
        } else {
          this.logger.warn(`[WRONG]mq is consuming, so can not unlock it, ${mq.key}. maybe hanged for a while, ${pq.tryUnlockTimes}`);
  ...
  toRunningProperties() {
    return {
      ...
      consumeThreadMin: this.options.consumeConcurrentMin,
      consumeThreadMax: this.options.consumeConcurrentMin,
      subscription: '{}',
...
/**
 * @typedef {object} ConsumeMessageService
 * @property {number} corePoolSize
 * @property {() => void} close
 * @property {() => void} init
 * @property {(msg: Message, brokerName: string) => ConsumeMessageDirectlyResult} consumeMessageDirectly
 * @property {(msgs: string[], processQueue: ProcessQueue, messageQueue: MessageQueue, dispathToConsume: boolean) => void} submitConsumeRequest
 */
