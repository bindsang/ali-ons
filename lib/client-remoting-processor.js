...
module.exports = class ClientRemotingProcessor {
  ...
  _getConsumerRunningInfo(address, request) {
    ...
    // ConsumerRunningInfo
    const consumerRunningInfo = this._mqClient.consumerRunningInfo(requestHeader.consumerGroup);
  ...
  _consumeMessageDirectly(address, request) {
    ...
    const result = this._mqClient.consumeMessageDirectly(msg, requestHeader.consumerGroup, requestHeader.brokerName);
    if (result) {
      response.code = ResponseCode.SUCCESS;
