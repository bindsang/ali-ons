...
module.exports = class ClientRemotingProcessor {
  ...
  async processRequest(address, request) {
    switch (request.code) {
      ...
      case RequestCode.GET_CONSUMER_RUNNING_INFO:
        return this._getConsumerRunningInfo(address, request);
      case RequestCode.NOTIFY_CONSUMER_IDS_CHANGED:
        return this._notifyConsumerIdsChanged(address, request);
      case RequestCode.RESET_CONSUMER_CLIENT_OFFSET:
        return this._resetOffset(address, request);
      case RequestCode.GET_CONSUMER_STATUS_FROM_CLIENT:
        return this._getConsumeStatus(address, request);

      case RequestCode.CONSUME_MESSAGE_DIRECTLY:
        return this._consumeMessageDirectly(address, request);
      default:
        break;
    }
    return null;
  ...
  async _notifyConsumerIdsChanged(address, request) {
    try {
      ...
      await this.mqClient.doRebalance();
    } catch (err) {
  ...
  async _resetOffset(address, request) {
    ...
    if (request.body) {
      ...
      const jsonText = str.replace(MQ_REGEX, s => {
        const queue = MessageQueue.fromJSON(s);
    ...
    await this.mqClient.resetOffset(requestHeader.topic, requestHeader.group, offsetTable);
    return null;
  }
  ...
  _getConsumeStatus(address, request) {
     ...
     */
    const offsetTable = this.mqClient.getConsumerStatus(requestHeader.topic, requestHeader.group);
    ...
    for (const [ , item ] of offsetTable) {
      ...
      builder.push(item.messageQueue.toJSON());
      builder.push(':');
   ...
   * @return {RemotingCommand} -
   */
  _getConsumerRunningInfo(address, request) {
    const response = RemotingCommand.createResponseCommand();
    // GetConsumerRunningInfoRequestHeader
    const requestHeader = request.decodeCommandCustomHeader();

    // ConsumerRunningInfo
    const consumerRunningInfo = this.mqClient.consumerRunningInfo(requestHeader.consumerGroup);
    if (consumerRunningInfo) {
      ...
      for (const [ , item ] of mqTable) {
        ...
        builder.push(item.messageQueue.toJSON());
        builder.push(':');
        builder.push(JSON.stringify(item.info));
      }
      builder.push('}');
      response.body = Buffer.from(builder.join(''));
      consumerRunningInfo.mqTable = '$$PLACE_HOLDER$$';
      const str = JSON.stringify(consumerRunningInfo);
      str.replace('"$$PLACE_HOLDER$$"', builder.toString());
   ...
   * @return {RemotingCommand} -
   */
  _consumeMessageDirectly(address, request) {
    const response = RemotingCommand.createResponseCommand();
    ...
    const result = this.mqClient.consumeMessageDirectly(msg, requestHeader.consumerGroup, requestHeader.brokerName);
    if (result) {
      response.code = ResponseCode.SUCCESS;
