...
const ClientRemotingProcessor = require('./client-remoting-processor');
...
class MQClientAPI extends RemotingClient {
  ...
  constructor(options) {
    ...
    this._processor = new ClientRemotingProcessor(this.logger, this);
    const listener = this.processMessageReceived.bind(this);
   ...
   * @param {number} maxConsumeRetryTimes -
   *
   */
  async consumerSendMessageBack(addr, msg, consumerGroup, delayLevel, timeoutMillis, maxConsumeRetryTimes) {
    const requestHeader = {
    ...
    const response = await this.invoke(
      MixAll.brokerVIPChannel(this.options.vipChannelEnabled, addr), request, timeoutMillis);
  ...
  /**
   * @param {string} addr -
   * @param {UnlockBatchRequestBody} requestBody -
   * @param {number} timeoutMillis -
   * @param {boolean} oneway -
   * @return {Promise<void>} -
   */
  async unlockBatchMQ(addr, requestBody, timeoutMillis, oneway) {
    const request = RemotingCommand.createRequestCommand(RequestCode.UNLOCK_BATCH_MQ, null);
    requestBody.mqSet = requestBody.mqSet.map(item => item.toJSON());
    request.body = Buffer.from(JSON.stringify(requestBody));

    if (oneway) {
      await this.invokeOneway(addr, request);
    } else {
      const response = await this.invoke(MixAll.brokerVIPChannel(this.options.vipChannelEnabled, addr), request, timeoutMillis);
      switch (response.code) {
        case ResponseCode.SUCCESS:
          return;
        default:
      ...
      throw new Error(`CODE: ${response.code}, DESC: ${response.remark}`);
    }
  }

  /**
   * @param {string} addr -
   * @param {LockBatchRequestBody} requestBody -
   * @param {number} timeoutMillis -
   * @return {Promise<MessageQueue[]>} -
   */
  async lockBatchMQ(addr, requestBody, timeoutMillis) {
    const request = RemotingCommand.createRequestCommand(RequestCode.LOCK_BATCH_MQ, null);
    requestBody.mqSet = requestBody.mqSet.map(item => item.toJSON());
    request.body = Buffer.from(JSON.stringify(requestBody));
    ...
    switch (response.code) {
      case ResponseCode.SUCCESS: {
        const body = response.body;
        const responseBody = JSON2.parse(body.toString());
        const messageQueues = responseBody.lockOKMQSet.map(item => new MessageQueue(item.topic, item.brokerName, item.queueId));
  ...
  async processMessageReceived(request, address) {
    const cmd = request.data;
    if (cmd && cmd.type === 'REQUEST_COMMAND') {
      await this.processRequestCommand(cmd, address);
    }
  }
   ...
   *
   * @param {RemotingCommand} cmd request command.
   * @param {string} address broker address
   */
  async processRequestCommand(cmd, address) {
    const opaque = cmd.opaque;

    try {
      const response = await this._processor.processRequest(address, cmd);
      if (!cmd.isOnewayRPC && response) {
        response.opaque = opaque;
        response.markResponseType();
        try {
          await this.invoke(address, response);
        } catch (err) {
          this.logger.error('process request over, but response failed', err);
          this.logger.error(JSON2.stringify(cmd));
          this.logger.error(JSON2.stringify(response));
        }
      }
    } catch (err) {
      this.logger.error('process request exception', err);
      this.logger.error(JSON2.stringify(cmd));

      if (!cmd.isOnewayRPC) {
        const response = RemotingCommand.createResponseCommand(ResponseCode.SYSTEM_ERROR,
          err.message + ' -> ' + err.stack);
        response.opaque = opaque;
        await this.invokeOneway(address, response);
      }
    }
  }
