...
class MQClientAPI extends RemotingClient {
  ...
  constructor(options) {
    ...
    this.on('request', (request, address) => {
      const cmd = request.data;
      if (cmd && cmd.type === 'REQUEST_COMMAND') {
        // 这里不使用await，使用异步响应的方式，防止请求长时间阻塞
        this.processRequestCommand(cmd, address);
      }
    });
  ...
  async consumerSendMessageBack(brokerAddr, msg, consumerGroup, delayLevel, timeoutMillis, maxConsumeRetryTimes) {
    ...
    brokerAddr = MixAll.brokerVIPChannel(this.options.vipChannelEnabled, brokerAddr);
    const response = await this.invoke(brokerAddr, request, timeoutMillis);
    switch (response.code) {
      case ResponseCode.SUCCESS:
        return;
