...
class MQClient extends MQClientAPI {
  ...
  async sendHeartbeatToAllBroker() {
    ...
    for (const brokerName of this._brokerAddrTable.keys()) {
      ...
      for (const id in oneTable) {
        ...
        brokers.push({ brokerName, id, addr });
      }
    }
    const ret = await gather(brokers.map(({ brokerName, id, addr }) => async () => {
      try {
        await this.sendHearbeat(addr, heartbeatData, 3000);
      } catch (e) {
        if (this._isBrokerInNameServer(addr)) {
          this.logger.info('send heart beat to broker[%s %s %s] failed', brokerName, id, addr);
        } else {
          this.logger.info('send heart beat to broker[%s %s %s] exception, because the broker not up, forget it', brokerName,
            id, addr);
        }
      }
    }));
   ...
   * @return {boolean} -
   */
  _isBrokerInNameServer(brokerAddr) {
    for (const [ , val ] of this._topicRouteTable.entries()) {
      const brokerDatas = val.brokerDatas;
      for (const bd of brokerDatas) {
        const contain = Object.values(bd.brokerAddrs).includes(brokerAddr);
        if (contain) {
          return true;
        }
      }
  ...
  async consumeMessageDirectly(msg, consumerGroup, brokerName) {
    ...
    if (consumer) {
       ...
       */
      const service = consumer._consumeMessageService;
      const result = await service.consumeMessageDirectly(msg, brokerName);
      return result;
    }
