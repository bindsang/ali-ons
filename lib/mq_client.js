...
class MQClient extends MQClientAPI {
   ...
   * @constructor
   */
  constructor(clientConfig, clientId) {
    super(clientConfig);
    ...
    // this.API.on('command', this.handleServerRequest.bind(this));  // 创建内置的发送消息的队列
    // --------------- add by zhangbing begin ---------------
    this._clientId = clientId;
    // 不使用require的目的是避免循环依赖造成Producer获取到的值不正确
    const Producer = require.cache[require.resolve('./producer/mq_producer')].exports;
    this._defaultMQProducer = new Producer({ ...clientConfig, producerGroup: MixAll.CLIENT_INNER_PRODUCER_GROUP }, false);
    // --------------- add by zhangbing end ---------------
  }

  // --------------- add by zhangbing begin ---------------
  getDefaultMQProducer() {
    return this._defaultMQProducer;
  }
  // --------------- add by zhangbing end ---------------
  ...
  async init() {
    ...
    this._defaultMQProducer.init(false);
    this._rebalancing = false;


    this.startScheduledTask('updateAllTopicRouterInfo', this.pollNameServerInteval, 10);
    this.startScheduledTask('sendHeartbeatToAllBroker', this.heartbeatBrokerInterval, 1000);
  ...
  static getAndCreateMQClient(clientConfig) {
    ...
    if (!instance) {
      instance = new MQClient(clientConfig.cloneClientConfig(), clientId);
  ...
  // ----------- add by zhangbing beging ---------------
  /**
   * @param {string} topic -
   * @param {string} group -
   * @param {Map<string, number>} offsetTable -
   * @return {Promise<void>} -
   */
  async resetOffset(topic, group, offsetTable) {
    const consumer = this._consumerTable.get(group);
    if (!consumer) {
      this.logger.info(`[reset-offset] consumer dose not exist. group=${group}`);
      return;
    }
    try {
      consumer.suspend();
      ...
      /**
       * @type {Map<string, { messageQueue: MessageQueue, processQueue: ProcessQueue, queueOffset: number}>}
       */
      const processQueueTable = consumer.processQueueTable;
      for (const [ key, item ] of processQueueTable) {
        const mq = item.messageQueue;
        if (topic === mq.topic && offsetTable.has(key)) {
      ...
      const keys = [ ...processQueueTable.keys() ];
      for (const key of keys) {
        const item = processQueueTable.get(key);
        const mq = item.messageQueue;
        const offset = offsetTable.get(key);
        if (topic === mq.topic && offset) {
          try {
            consumer._offsetStore.updateOffset(mq, offset);
            await consumer.removeUnnecessaryMessageQueue(mq, item.processQueue);
            processQueueTable.delete(key);
          } catch (err) {
            this.logger.warn(`reset offset failed. group=${group}, ${mq.key}`, err);
          }
        }
      }
    } finally {
  ...
  getConsumerStatus(topic, group) {
    ...
    if (consumer) {
      const mqs = [];
      for (const [ , item ] of consumer.processQueueTable) {
        mqs.push(item.messageQueue);
      }
      return consumer._offsetStore.cloneOffsetTable(mqs);
    }
    return new Map();
  }

  async consumerRunningInfo(consumerGroup) {
    const consumer = this._consumerTable.get(consumerGroup);
    const consumerRunningInfo = await consumer.consumerRunningInfo();
    const nsList = this._namesrvAddrList;
    consumerRunningInfo.properties.PROP_NAMESERVER_ADDR = nsList.join(';');
    consumerRunningInfo.properties.PROP_CONSUME_TYPE = consumer.consumeType;
    consumerRunningInfo.properties.PROP_CLIENT_VERSION = 'v4.3.2-mctech';

    return consumerRunningInfo;
  }

  async consumeMessageDirectly(msg, consumerGroup, brokerName) {
    const consumer = this._consumerTable.get(consumerGroup);
    if (consumer) {
      const result = await consumer._consumeMessageService.consumeMessageDirectly(msg, brokerName);
      return result;
    }
